...
function appendChangelogEntry(
  ...
  try {
    ...
    // Ownership guard: only write into a changelog this tool created. On
    // Windows 'Changelog.md' and a hand-maintained 'CHANGELOG.md' are the
    // SAME file (case-insensitive filesystem), and a Keep-a-Changelog file
    // uses '## [x.y.z.w]' headings this appender doesn't understand -- it
    // would dump its entry at the end of the file. Leave foreign files alone.
    if (existing.trim() && !existing.includes('Auto-generated by `bump_project_version`')) {
      serverLog.warn(
        `Changelog append skipped: ${changelogPath} exists but was not created by ` +
          `bump_project_version (hand-maintained changelog?) -- record the bump there yourself`
      );
      return;
    }
